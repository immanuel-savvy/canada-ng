...
import { categories } from "./New_seminar";

class New_conference extends Handle_file_upload {
  constructor(props) {
    super(props);
    ...
    if (window.location.pathname === "/edit_seminar")
      conference = conference || get_session("seminar_in_edit");
    ...
    this.state = {
      ...
      vendor: null,
      date: conference && new Date(conference.date),
      price: (conference && conference.value) || "",
    };
  }
  ...
  handle_seminar = () => {
    let { new_conference: conference } = this.state;
    window.sessionStorage.setItem("conference", JSON.stringify(conference));
  };

  finish_tab_panel = () => {
    let { uploading_seminar, new_conference, _id } = this.state;
        ...
        {!this.is_set() && !new_conference ? (
          ...
          <div className="succ_wrap">
            ...
            <div className="succ_122">
              <h4>Conference Successfully {_id ? "Updated" : `Added`}</h4>
              <p>{new_conference?.short_description}</p>
            </div>
            <div className="succ_123">
              <Link to={`/conference?${new_conference?._id}`}>
                <span
  ...
  on_finish = async () => {
    ...
    if (things_to_know.length) conference.things_to_know = things_to_know;
    if (what_to_expect.length) conference.what_to_expect = what_to_expect;

    let response;
    if (_id) {
      conference._id = _id;
      response = await post_request("update_conference", conference);
      conference.images = response.images;
    } else {
      response = await post_request("new_conference", conference);
      conference.images = response.images;
      conference._id = response._id;
      conference.created = response.created;
    }

    if (response?._id) {
      this.setState({ new_conference: conference });

      emitter.emit(_id ? "conference_updated" : "new_conference", {
        ...conference,
      });
      this.reset_state();
    }
        ...
        {({ loggeduser }) => {
          ...
          return (
            <div id="main-wrapper">
              <Custom_Nav />
              <Padder />
              <Breadcrumb_banner page="new conference" />

              <div className="container pt-5">
                <Section_header title="New Conference" description="" />
